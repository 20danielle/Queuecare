...
if (!defined('QR_PAD0')) define("QR_PAD0", 0xEC);
if (!defined('QR_PAD1')) define("QR_PAD1", 0x11);
...
class QRCodeKaz {
    ...
    function makeImpl($test, $maskPattern) {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = QRCodeKaz::createNullArray($this->moduleCount);
        }
        ...
        $data = QRCodeKaz::createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        return QRCodeKaz::createBytes($buffer, $rsBlocks);
    }
    ...
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = QRCodeKaz::createNullArray(count($rsBlocks) );
        $ecdata = QRCodeKaz::createNullArray(count($rsBlocks) );
        ...
        for ($r = 0; $r < $rsBlockCount; $r++) {
            ...
            $dcdata[$r] = QRCodeKaz::createNullArray($dcCount);
            ...
            $ecdata[$r] = QRCodeKaz::createNullArray($rsPoly->getLength() - 1);
        ...
        $data = QRCodeKaz::createNullArray($totalCodeCount);
...
if (!defined("QR_G15")) define("QR_G15", (1 << 10) | (1 << 8) | (1 << 5)
    | (1 << 4) | (1 << 2) | (1 << 1) | (1 << 0) );

if (!defined("QR_G18")) define("QR_G18", (1 << 12) | (1 << 11) | (1 << 10)
    | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2) | (1 << 0) );

if (!defined("QR_G15_MASK")) define("QR_G15_MASK", (1 << 14) | (1 << 12) | (1 << 10)
    | (1 << 4) | (1 << 1) );
...
if (!defined("QR_MODE_NUMBER")) define("QR_MODE_NUMBER", 1 << 0);
if (!defined("QR_MODE_ALPHA_NUM")) define("QR_MODE_ALPHA_NUM", 1 << 1);
if (!defined("QR_MODE_8BIT_BYTE")) define("QR_MODE_8BIT_BYTE", 1 << 2);
if (!defined("QR_MODE_KANJI")) define("QR_MODE_KANJI", 1 << 3);

//---------------------------------------------------------------
// MaskPattern
//---------------------------------------------------------------

if (!defined("QR_MASK_PATTERN000")) define("QR_MASK_PATTERN000", 0);
if (!defined("QR_MASK_PATTERN001")) define("QR_MASK_PATTERN001", 1);
if (!defined("QR_MASK_PATTERN010")) define("QR_MASK_PATTERN010", 2);
if (!defined("QR_MASK_PATTERN011")) define("QR_MASK_PATTERN011", 3);
if (!defined("QR_MASK_PATTERN100")) define("QR_MASK_PATTERN100", 4);
if (!defined("QR_MASK_PATTERN101")) define("QR_MASK_PATTERN101", 5);
if (!defined("QR_MASK_PATTERN110")) define("QR_MASK_PATTERN110", 6);
if (!defined("QR_MASK_PATTERN111")) define("QR_MASK_PATTERN111", 7);

//---------------------------------------------------------------
// ErrorCorrectLevel

// 7%.
if (!defined("QR_ERROR_CORRECT_LEVEL_L")) define("QR_ERROR_CORRECT_LEVEL_L", 1);
// 15%.
if (!defined("QR_ERROR_CORRECT_LEVEL_M")) define("QR_ERROR_CORRECT_LEVEL_M", 0);
// 25%.
if (!defined("QR_ERROR_CORRECT_LEVEL_Q")) define("QR_ERROR_CORRECT_LEVEL_Q", 3);
// 30%.
if (!defined("QR_ERROR_CORRECT_LEVEL_H")) define("QR_ERROR_CORRECT_LEVEL_H", 2);

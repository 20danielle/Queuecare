...
(function() {
    ...
    // ── Export PDF (téléchargement réel, sans boîte de dialogue d'impression) ──
    window.admExporterPDF = function() {
        const hopital = <?= json_encode($nomHopitalStat) ?>;
        const dateAffichage = new Date().toLocaleDateString('fr-FR', { day:'2-digit', month:'long', year:'numeric' });
        ...
        if (window._admDataAtt && window._admDataAtt.par_sous_service) {
            window._admDataAtt.par_sous_service.forEach(ss => {
                ...
                ssRows.push([
                    ss.ss_nom,
                    ss.attente_moy_min != null ? ss.attente_moy_min + ' min' : '—',
                    ss.attente_7j_min != null ? ss.attente_7j_min + ' min' : '—',
                    ss.attente_30j_min != null ? ss.attente_30j_min + ' min' : '—',
                    tendTxt,
                    String(ss.nb_mesures || 0)
                ]);
            });
        }
        ...
        if (_data && _data.evolution_globale) {
            _data.evolution_globale.slice(-30).forEach(r => {
                evRows.push([r.jour, String(r.total||0), String(r.traitees||0), String(r.absentes||0), String(r.annulees||0)]);
            });
        }
        ...
        function drawCards(cards) {
            const cardW = (pageWidth - marginX * 2 - (cards.length - 1) * 10) / cards.length;
            const cardH = 50;
            ...
            cards.forEach((c, i) => {
                ...
                doc.setFontSize(14);
                doc.setTextColor(blue[0], blue[1], blue[2]);
                doc.text(String(c.val), x + cardW / 2, y + 22, { align: 'center' });
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(7.5);
                doc.setTextColor(gray[0], gray[1], gray[2]);
                doc.text(c.lbl, x + cardW / 2, y + 36, { align: 'center' });
            });
            y += cardH + 20;
        }
        ...
        function drawTable(headers, rows) {
            ...
            const colW = (pageWidth - marginX * 2) / colCount;
            const rowH = 18;
            ...
            headers.forEach((h, i) => doc.text(h, marginX + i * colW + 6, y + 12));
            y += rowH;
            doc.setFont('helvetica', 'normal');
            if (rows.length === 0) {
                checkPageBreak(rowH);
                doc.setTextColor(gray[0], gray[1], gray[2]);
                doc.setFontSize(8);
                doc.text('Aucune donnée.', pageWidth / 2, y + 12, { align: 'center' });
                y += rowH;
                return;
            }
            rows.forEach((row, ri) => {
                ...
                doc.setTextColor(30, 41, 59);
                doc.setFontSize(8);
                row.forEach((cell, i) => doc.text(String(cell), marginX + i * colW + 6, y + 12));
                y += rowH;
            });
            y += 12;
        }
        ...
        doc.setFontSize(17);
        doc.setTextColor(blue[0], blue[1], blue[2]);
        doc.text('Rapport de statistiques', marginX, y);
        y += 18;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(11);
        doc.setTextColor(30, 41, 59);
        doc.text(hopital, marginX, y);
        y += 14;
        doc.setFontSize(8.5);
        doc.setTextColor(gray[0], gray[1], gray[2]);
        doc.text(`Généré le ${dateAffichage} · Période : ${periode}`, marginX, y);
        y += 24;

        drawTitle('Consultations globales');
        drawCards([
            { val: tot, lbl: 'Total' },
            { val: tr, lbl: 'Traitées' },
            { val: ab, lbl: 'Absents' },
            { val: an, lbl: 'Annulées' },
            { val: dm, lbl: 'Durée moy.' },
            { val: tx, lbl: 'Taux traitement' }
        ]);

        drawTitle("Temps d'attente moyen");
        drawCards([
            { val: ag, lbl: 'Global' },
            { val: a7, lbl: '7 derniers jours' },
            { val: a30, lbl: '30 derniers jours' },
            { val: nb, lbl: 'Consultations mesurées' }
        ]);

        drawTitle('Attente par sous-service');
        drawTable(['Sous-service', 'Globale', '7j', '30j', 'Tendance', 'Mesures'], ssRows);

        drawTitle('Évolution des consultations (30 derniers jours)');
        drawTable(['Date', 'Total', 'Traitées', 'Absents', 'Annulées'], evRows);
        ...
        for (let p = 1; p <= pageCount; p++) {
            ...
            doc.text('QueueCare — Rapport généré le ' + dateAffichage, marginX, doc.internal.pageSize.getHeight() - 20);
            doc.text(`Page ${p}/${pageCount}`, pageWidth - marginX, doc.internal.pageSize.getHeight() - 20, { align: 'right' });
        }

        const nomFichier = `stats_${hopital.replace(/[^a-z0-9]+/gi,'_')}_${new Date().toISOString().slice(0,10)}.pdf`;
        doc.save(nomFichier);
    };

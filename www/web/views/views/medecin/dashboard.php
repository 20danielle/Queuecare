...
if (isset($medecin) && !empty($medecin['photo']) && $medecin['photo'] !== 'null') {
    $cheminPhysique = __DIR__ . '/../../public/' . $medecin['photo'];
    if (file_exists($cheminPhysique)) {
        $photoUrl = 'public/' . $medecin['photo'];
        $photoValide = true;
    }
}
...
<head>
    ...
    <style>
        ...
        .sidebar { width: 260px; background: linear-gradient(180deg, #0a2540 0%, #0f2c3f 100%); color: white; display: flex; flex-direction: column; position: fixed; height: 100vh; overflow: hidden; transition: transform 0.3s ease; z-index: 200; }
...
<div class="app-container">
    ...
    <main class="main-content">
        ...
        <div class="dashboard-content">
            ...
            <!-- Rappel périodique : consultation en cours non terminée -->
            <div id="rappelConsultationBanner" style="display:none;position:fixed;top:64px;left:16px;right:16px;z-index:9998;background:#fffbeb;border:1.5px solid #f59e0b;color:#92400e;padding:12px 18px;align-items:center;gap:16px;font-size:.85rem;font-weight:600;box-shadow:0 8px 24px rgba(0,0,0,.15);border-radius:10px;max-width:560px;margin:0 auto;">
                <span style="display:flex;align-items:center;gap:10px;">
                    <i class="fa-solid fa-bell" style="color:#d97706;font-size:1.05rem;"></i>
                    <span id="rappelConsultationTexte">Vous avez une consultation en cours. N'oubliez pas de la terminer avant d'en démarrer une nouvelle.</span>
                </span>
            </div>
                    ...
                    <div style="overflow-x:auto;">
                        <table class="med-table">
                            ...
                            <tbody>
                                ...
                                <tr data-id="<?= $c['id'] ?>">
                                    <td data-label="#"><div class="rang-badge <?= $c['statut'] === 'en_cours' ? 'current' : '' ?>"><?= (int)$c['rang'] ?></div></td>
                                    ...
                                    <td data-label="Heure prévue"><?= isset($c['heure_passage_estimee']) && $c['heure_passage_estimee'] ? date('H:i', strtotime($c['heure_passage_estimee'])) : '—' ?></td>
                                    <td data-label="Heure début"><?= isset($c['heure_debut_reelle']) && $c['heure_debut_reelle'] ? date('H:i', strtotime($c['heure_debut_reelle'])) : '—' ?></td>
                                    <td data-label="Heure fin"><?= isset($c['heure_fin_reelle']) && $c['heure_fin_reelle'] ? date('H:i', strtotime($c['heure_fin_reelle'])) : '—' ?></td>
                                    ...
                                    <td data-label="Action"><div style="display:flex; gap:5px;">
...
<script>
    ...
    /**
     * Affiche un message d'erreur DIRECTEMENT à l'intérieur d'une modale
     * ouverte (au lieu du toast global qui peut être masqué derrière elle).
     * @param {string} erreurId   id de l'élément .med-modal-error
     * @param {string} texteId    id du <span> qui contient le texte
     * @param {string} message
     */
    function afficherErreurModale(erreurId, texteId, message) {
    ...
    function cacherErreurModale(erreurId) {
        const box = document.getElementById(erreurId);
    ...
    function afficherConsultationsMed(cons, filtered) {
        ...
        for(const v of page) {
            ...
            h += `<tr ${rowStyle}>
                <td data-label="#"><div class="rang-badge ${v.statut === 'en_cours' ? 'current' : ''}">${v.rang_medecin ?? v.rang}</div></td>
                ...
                <td data-label="Heure prévue">${v.heure_passage_estimee || '—'}</td>
                <td data-label="Heure début">${v.heure_debut_reelle || '—'}</td>
                <td data-label="Heure fin">${v.heure_fin_reelle || '—'}</td>
                <td data-label="Statut"><span class="badge ${bc}">${lb}</span>${pauseDetail}</td>
                <td data-label="Action"><div style="display:flex; gap:5px; flex-wrap:wrap;">`;
    ...
    function ouvrirModalPause(id) {
        ...
        cacherErreurModale('pauseErreur');
        if(modal) {
    ...
    function confirmerPause() {
        ...
        cacherErreurModale('pauseErreur');
        const btn = document.getElementById('pauseBtnConfirmer');
        ...
        .then(r => r.json()).then(d => {
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-pause"></i> Confirmer la pause'; }
            if(d.success) {
                fermerModalPause();
                afficherMessage('Consultation mise en pause — ' + motif, 'success');
                setTimeout(rafraichirConsultations, 600);
            } else {
                // Erreur affichée DANS la modale : pas besoin de la fermer pour la lire
                afficherErreurModale('pauseErreur', 'pauseErreurTexte', d.message || 'Erreur lors de la mise en pause.');
            }
        })
        .catch(() => {
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-pause"></i> Confirmer la pause'; }
            afficherErreurModale('pauseErreur', 'pauseErreurTexte', 'Erreur réseau. Veuillez réessayer.');
        });
    }
    ...
    function afficherTableauHistorique(rows) {
        ...
        for(const v of rows) {
            ...
            h += `<tr>
                ...
                <td data-label="Statut"><span class="badge ${bcMap[v.statut]||'badge-secondary'}">${lbMap[v.statut]||v.statut}</span></td>
    ...
    function afficherFormulaireProfil(profil) {
        const pp = profil.photo && profil.photo !== 'null' && profil.photo !== '' ? 'public/' + profil.photo : 'public/images/default-avatar.png';
...
function dessinerChartTempsAttente(evolution, ssNom) {
    ...
    /* ══════════════════════════════════
       Rappel périodique : consultation en cours non terminée.
       Tant qu'une consultation est "en_cours", un rappel s'affiche
       toutes les 10 minutes (comptées depuis son heure de début réelle,
       pas depuis l'ouverture de la page) pour que le médecin n'oublie
       pas de la clôturer avant d'en démarrer une autre.
    ══════════════════════════════════ */
    const RAPPEL_INTERVALLE_MIN = 10;
    let _dernierPalierRappele = null; // évite de réafficher le même palier de 10 min en boucle

    function verifierRappelConsultationEnCours() {
        const enCours = (_consultationsDataMed || []).find(c => c.statut === 'en_cours');
        const banner  = document.getElementById('rappelConsultationBanner');
        if (!banner) return;

        if (!enCours) {
            // Plus aucune consultation en cours : on cache le rappel et on
            // réinitialise pour la prochaine consultation démarrée.
            banner.style.display = 'none';
            _dernierPalierRappele = null;
            return;
        }

        // Durée écoulée depuis le début réel de la consultation (heure_debut_reelle
        // est au format "HH:MM" côté JS, cf. _consultationsDataMed).
        if (!enCours.heure_debut_reelle) return;
        const [h, m] = enCours.heure_debut_reelle.split(':').map(Number);
        const debut = new Date();
        debut.setHours(h, m, 0, 0);
        let minutesEcoulees = Math.floor((Date.now() - debut.getTime()) / 60000);
        if (minutesEcoulees < 0) minutesEcoulees += 24 * 60; // sécurité passage minuit

        const palierActuel = Math.floor(minutesEcoulees / RAPPEL_INTERVALLE_MIN);

        if (minutesEcoulees >= RAPPEL_INTERVALLE_MIN && palierActuel !== _dernierPalierRappele) {
            _dernierPalierRappele = palierActuel;
            const texte = document.getElementById('rappelConsultationTexte');
            if (texte) {
                texte.textContent = `Consultation avec ${enCours.patient_prenom || ''} ${enCours.patient_nom || ''} en cours depuis ${minutesEcoulees} min. N'oubliez pas de la terminer avant d'en démarrer une nouvelle.`;
            }
    ...
    // Vérification chaque minute (le déclenchement réel reste cadencé sur
    // des paliers de 10 minutes grâce à _dernierPalierRappele ci-dessus).
    setInterval(verifierRappelConsultationEnCours, 60 * 1000);
    ...
    window.onload = function() {
        ...
        verifierRappelConsultationEnCours();
    };
...
<!-- Modal Pause Examen -->
<div id="modalPause" class="med-modal-backdrop">
    <div class="med-modal-content" style="max-width:420px;">
        ...
        <!-- Erreur (visible directement dans la modale, sans avoir à la fermer) -->
        <div id="pauseErreur" class="med-modal-error">
            <i class="fa-solid fa-circle-exclamation" style="margin-top:1px;"></i>
            <span id="pauseErreurTexte"></span>
        </div>
        ...
        <p style="font-size:.78rem;color:#94a3b8;margin-bottom:16px;">
            ...
            Le patient restera "en pause" jusqu'à son retour, sans limite de temps — vous pourrez le reprendre ou le marquer absent manuellement à tout moment.
        </p>
        <div style="display:flex;gap:10px;justify-content:flex-end;">
            ...
            <button onclick="confirmerPause()" id="pauseBtnConfirmer" style="padding:9px 18px;background:#f59e0b;border:none;border-radius:8px;cursor:pointer;font-size:.85rem;font-family:inherit;color:#fff;font-weight:600;">
...
══════════════════════════════════════════ -->
<div id="modalRdv" class="med-modal-backdrop">
    <div class="med-modal-content" style="max-width:480px;">
        ...
        <!-- Erreur (visible directement dans la modale, sans avoir à la fermer) -->
        <div id="rdvErreur" class="med-modal-error">
            <i class="fa-solid fa-circle-exclamation" style="margin-top:1px;"></i>
            <span id="rdvErreurTexte"></span>
        </div>
...
function confirmerRdv() {
    ...
    if (!date) {
        afficherErreurModale('rdvErreur', 'rdvErreurTexte', 'Veuillez sélectionner une date.');
        return;
    }
    if (!_rdvHeureSelectionnee) {
        afficherErreurModale('rdvErreur', 'rdvErreurTexte', 'Veuillez sélectionner un créneau horaire.');
        return;
    }
    ...
    .then(d => {
        ...
        if (d.success) {
            ...
            // Erreur affichée DANS la modale : pas besoin de la fermer pour la lire
            afficherErreurModale('rdvErreur', 'rdvErreurTexte', d.message || 'Impossible de créer le rendez-vous.');
        }
    })
    .catch(() => {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-calendar-check"></i> Confirmer le RDV';
        afficherErreurModale('rdvErreur', 'rdvErreurTexte', 'Erreur réseau. Veuillez réessayer.');
    });
}

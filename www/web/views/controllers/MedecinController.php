...
class MedecinController
{
    ...
    public function getStatsData(): void
    {
        ...
        $stats = $this->model->statsJour($affectation["ss_id"], $medecinId);
    ...
    public function demarrerConsultationAjax(): void
    {
        ...
        $result = $this->model->demarrerConsultation($consultationId);
    ...
    public function mettreAJourProfil(): void
    {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            $upload = UploadHelper::uploadPhoto($_FILES['photo'], 'medecins');
            if ($upload['success']) {
                if ($medecin['photo'] && file_exists(__DIR__ . '/../public/' . $medecin['photo'])) {
                    unlink(__DIR__ . '/../public/' . $medecin['photo']);
                }

...
class ConsultationModel
{
    ...
    /**
     * Calcule le prochain rang disponible pour un sous-service, pour une
     * date de PASSAGE donnée (utile pour les RDV pris en ligne à l'avance,
     * contrairement à getProchainRang() qui se base sur la date d'émission
     * du jour même).
     */
    public function getProchainRangPourDate(int $sousServiceId, string $datePassage): int
    {
        $stmt = $this->db->prepare(
        ...
        $stmt->execute([':ss_id' => $sousServiceId, ':date' => $datePassage]);
        $result = $stmt->fetch();

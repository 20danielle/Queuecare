...
class MedecinModel
{
    ...
    public function creer(array $d): int|false
    {
        ...
        // pas le booléen de execute(), sinon tous les nouveaux médecins
        // se retrouvent associés à l'ID 1 (true => (int)1) et héritent
        // des données du premier médecin créé.
    ...
    public function statsJour(int $ssId, int $medecinId = 0): array
    {
        if ($medecinId > 0) {
            $stmt = $this->db->prepare(
                'SELECT
    ...
    public function consultationsDuJour(int $ssId, int $medecinId): array
    {
        ...
        $stmt->execute([':ss' => $ssId, ':mid' => $medecinId]);
        $rows = $stmt->fetchAll();
        // Recalculate rang per this doctor's queue (1-based position)
    ...
    public function demarrerConsultation(int $consultationId): array
    {
        ...
        try {
            // Un médecin ne peut avoir qu'une seule consultation "en_cours" à la fois,
            // POUR UN MÊME JOUR. (Une consultation "en_pause" reste autorisée en
            // parallèle : c'est le système de pause pour examen externe,
            // volontairement laissé intact.)
            // Sans le filtre sur la date, une consultation "en_cours" oubliée
            // (jamais terminée) un jour précédent bloquait indéfiniment tout
            // nouveau démarrage pour ce médecin, même si elle n'apparaît plus
            // dans le dashboard du jour (qui filtre déjà par date).
            // FOR UPDATE verrouille les lignes concernées pour empêcher un
            // double démarrage en cas de double-clic / requêtes simultanées.
            $stmtCheck = $this->db->prepare(
                'SELECT c2.id
                 FROM consultations c1
                 JOIN consultations c2 ON c2.medecin_id = c1.medecin_id
                                       AND c2.id <> c1.id
                                       AND c2.statut = "en_cours"
                                       AND DATE(c2.heure_passage_estimee) = DATE(c1.heure_passage_estimee)
                 WHERE c1.id = :id
                 FOR UPDATE'
            );
            $stmtCheck->execute([':id' => $consultationId]);
            if ($stmtCheck->fetch()) {
                if (!$dejaEnTransaction) $this->db->rollBack();
                return ['success' => false, 'message' => 'Vous avez déjà une consultation en cours. Terminez-la ou mettez-la en pause avant d\'en démarrer une autre.'];
            }
            ...
            $ok = $stmt->execute([':id' => $consultationId]) && $stmt->rowCount() > 0;
            ...
            return $ok
                ? ['success' => true, 'message' => 'Consultation démarrée']
                : ['success' => false, 'message' => 'Impossible de démarrer cette consultation'];
        } catch (\Throwable $e) {
     ...
     * ne redevient "absent" que si le médecin le marque manuellement, ou
     * reprend normalement via "reprendreConsultation".)
        ...
        $stmt = $this->db->prepare(
            "SELECT c.id, c.rang,
                    (SELECT COUNT(*) + 1
                     ...
                     WHERE c2.medecin_id = c.medecin_id
                       AND DATE(c2.heure_passage_estimee) = DATE(c.heure_passage_estimee)
                       AND c2.rang < c.rang) AS rang_medecin,
                    c.statut, c.mode_prise, c.motif,
    ...
    public function fixerProchainRdv(int $consultationSourceId, string $dateRdv, string $heureRdv, string $motif = ''): array
    {
        ...
        try {
            // 2. Verrou de ligne + vérification : empêche deux requêtes
            //    simultanées de planifier toutes les deux un RDV pour le
            //    même patient, même service, même jour.
            ...
            // 4. Utiliser l'heure choisie par le médecin (et non la logique de file séquentielle,
            //    qui sert aux arrivées du jour même, pas à un RDV programmé à une heure précise)
        ...
        //    (hors transaction : reserverCreneauEdt gère son propre INSERT...ON DUPLICATE)
